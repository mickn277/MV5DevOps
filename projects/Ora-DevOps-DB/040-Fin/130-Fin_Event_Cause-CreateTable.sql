...
--
-- Purpose:
--  This script creates table Fin_Event_Cause, a new table in the database.
--  Foreign Keys on this table Fin_Event_Cause reference ReferencedTableName1, ReferencedTableName2 etc.
--  Foreign Keys on Fin_Events, ReferencingTableName2 reference this table Fin_Event_Cause.
--  
...
WHERE rdcn.owner = USER 
    AND rdcn.table_name = UPPER('Fin_Event_Cause')
...
DECLARE
    v_Exists NUMBER(1) := 0;
BEGIN 
    SELECT DECODE(table_name, null, 0, -1) TABLE_EXISTS INTO V_Exists FROM dual LEFT JOIN user_tables ON (table_name=UPPER('Fin_Event_Cause'));
    IF (v_Exists=-1) THEN
...
WHERE table_name = UPPER('Fin_Event_Cause')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Fin_Event_Cause'
-- --------------------
SELECT 
...
DECLARE
    ...
    END;
BEGIN
    ExecSQL('ALTER TABLE fin_events DROP CONSTRAINT fin_events_fk11');
END;
/
...
--DROP INDEX  ... Dropped when table is dropped.
	ExecSql('DROP SEQUENCE Fin_Event_Cause_sq');
	ExecSql('DROP TABLE Fin_Event_Cause');
END;
/
...
------------------------------------------------------------------
COMMENT ON TABLE Fin_Event_Cause IS 'Only interested in finacially significant events.';

-- Run this after creating the table to generate a list of table column comments:
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Fin_Event_Cause');

COMMENT ON COLUMN Fin_Event_Cause.id IS 'PK';

COMMENT ON COLUMN Fin_Event_Cause.Short_Desc IS 'Required';
-- COMMENT ON COLUMN Fin_Event_Cause.ColumnNameUK2 IS 'UK 2of2';
-- COMMENT ON COLUMN Fin_Event_Cause.ColumnNameFK1 IS 'FK1';
-- COMMENT ON COLUMN Fin_Event_Cause.ColumnNameFK2 IS 'FK2';

COMMENT ON COLUMN Fin_Event_Cause.Created_By IS 'Auditing column';
COMMENT ON COLUMN Fin_Event_Cause.Created_Dt IS 'Auditing column';
COMMENT ON COLUMN Fin_Event_Cause.Changed_By IS 'Auditing column';
COMMENT ON COLUMN Fin_Event_Cause.Changed_Dt IS 'Auditing column';

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Fin_Event_Cause  MODIFY (id DEFAULT 0);

-- ALTER TABLE Fin_Event_Cause  MODIFY (ColumnNameUK1 DEFAULT 0);
-- ALTER TABLE Fin_Event_Cause  MODIFY (ColumnNameUK2 DEFAULT 0);
-- ALTER TABLE Fin_Event_Cause  MODIFY (ColumnNameFK1 DEFAULT 0);
-- ALTER TABLE Fin_Event_Cause  MODIFY (ColumnNameFK2 DEFAULT 0);

ALTER TABLE Fin_Event_Cause  MODIFY (Created_Dt DEFAULT sysdate);
ALTER TABLE Fin_Event_Cause  MODIFY (archive DEFAULT 0);
...
------------------------------------------------------------------
-- Primary Key Index
CREATE UNIQUE INDEX Fin_Event_Cause_pk ON Fin_Event_Cause (id);

-- Unique Key Index 1
-- CREATE UNIQUE INDEX Fin_Event_Cause_uk1 ON Fin_Event_Cause (ColumnNameUK1, ColumnNameUK2);

-- Foreign Key Index 1
-- CREATE INDEX Fin_Event_Cause_ix1 ON Fin_Event_Cause (ColumnNameFK1);

-- Foreign Key Index 2
-- CREATE INDEX Fin_Event_Cause_ix2 ON Fin_Event_Cause (ColumnNameFK2);

-- Tuning Index 3
-- CREATE INDEX Fin_Event_Cause_ix3 ON Fin_Event_Cause (ColumnName, ColumnName) [TABLESPACE TablespaceName_index];

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add Constraints for this table --'
------------------------------------------------------------------
ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_pk PRIMARY KEY (id) USING INDEX;

-- ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_uk1 UNIQUE (ColumnNameUK1, ColumnNameUK2) USING INDEX;

ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_c1 CHECK (archive IN (0, -1));

-- ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_c2 CHECK (ColumnName IN ('Value1', 'Value2', 'Value3'));

-- ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_c3 CHECK (ColumnName [<|>|=|!=] Value);
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Fin_Event_Cause, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Fin_Event_Cause, use "ON DELETE SET NULL".
--  If you want Fin_Event_Cause to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
-- ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_fk1 FOREIGN KEY (ColumnNameFK1) REFERENCES ReferencedTableName1 (ReferencedPK1) [ON DELETE SET NULL];

-- ALTER TABLE Fin_Event_Cause ADD CONSTRAINT Fin_Event_Cause_fk2 FOREIGN KEY (ColumnNameFK2) REFERENCES ReferencedTableName2 (ReferencedPK2) [ON DELETE CASCADE];

------------------------------------------------------------------
PROMPT '-- (CREATE SEQUENCE) Create the Sequence for this table --'
------------------------------------------------------------------
-- Simple Create Sequence:
CREATE SEQUENCE Fin_Event_Cause_sq 
MINVALUE 1 MAXVALUE 9999999999999999999999999999 
INCREMENT BY 1 START WITH 100 CACHE 10 NOORDER NOCYCLE;

---- Rebuild the sequence from table max value:
--SET SERVEROUTPUT ON
--DECLARE
--    v_START_WITH NUMBER := 1;
--    v_Fin_Event_Cause VARCHAR2(30) := 'Fin_Event_Cause';
--BEGIN
--    BEGIN
--        EXECUTE IMMEDIATE 'SELECT max(NVL(id,0))+1 FROM dual LEFT JOIN '||v_Fin_Event_Cause||' ON (1=1)' INTO v_START_WITH;
--        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Event_Cause||'_sq START_WITH='||v_START_WITH);
--    EXCEPTION WHEN OTHERS THEN
--        DBMS_OUTPUT.PUT_LINE(SQLERRM);
--    END;
--    BEGIN
--        EXECUTE IMMEDIATE 'DROP SEQUENCE '||v_Fin_Event_Cause||'_sq';
--        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Event_Cause||'_sq DROPPED');
--    EXCEPTION WHEN OTHERS THEN
--        NULL;
--    END;
--    EXECUTE IMMEDIATE 'CREATE SEQUENCE '||v_Fin_Event_Cause||'_sq MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH '||v_START_WITH||' CACHE 10 NOORDER NOCYCLE';
--    DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Event_Cause||'_sq CREATED');
--END;
--/
...
------------------------------------------------------------------
CREATE OR REPLACE TRIGGER Fin_Event_Cause_tr1 BEFORE INSERT OR UPDATE ON Fin_Event_Cause FOR EACH ROW
DECLARE
...
BEGIN
    ...
    IF INSERTING THEN
        IF (:NEW.id IS NULL) THEN
            SELECT Fin_Event_Cause_sq.NEXTVAL INTO :NEW.id FROM dual; 
        END IF;
...
------------------------------------------------------------------
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (14, 'Major Tax', '', 14);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (13, 'Tax', '', 13);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (12, 'Monetary Boom', '', 12);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (11, 'Population Boom', '', 11);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (10, 'Resource Supply Boom', '', 10);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (9, 'Monetary Growth', '', 9);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (8, 'Population Growth', '', 8);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (7, 'Supply Growth', '', 7);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (6, 'Monetary/Banking Crisis', '', 6);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (5, 'Regional War', '', 5);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (4, 'World War', '', 4);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (3, 'Local Famine', '', 3);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (2, 'Regional Famine', '', 2);
INSERT INTO Fin_Event_Cause (id, Short_Desc, Description, Order_id) VALUES (1, 'World Famine', '', 1);
COMMIT;

EXECUTE DBMS_STATS.GATHER_TABLE_STATS(ownname=>USER, tabname=>UPPER('Fin_Event_Cause'), cascade=>TRUE, estimate_percent=>DBMS_STATS.AUTO_SAMPLE_SIZE, method_opt=>'for all columns size auto');
...
------------------------------------------------------------------
--GRANT SELECT ON Fin_Event_Cause TO ReadOnlyRole;
--GRANT SELECT, DELETE, UPDATE, INSERT ON Fin_Event_Cause TO ReadWriteRole;
...
DECLARE
    ...
    END;
BEGIN
    ExecSQL('ALTER TABLE fin_events ADD CONSTRAINT fin_events_fk11 FOREIGN KEY (cause_id) REFERENCES fin_event_cause (id) ON DELETE SET NULL');
END;
/
