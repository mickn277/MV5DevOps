...
--
-- Purpose:
--  This script creates new table Geo_Continents, a new table in the database.
--  Foreign Keys on this new table Geo_Continents reference ReferencedTableName1, ReferencedTableName2 etc.
--
...
WHERE rdcn.owner = USER 
  AND rdcn.table_name = UPPER('Geo_Continents')
...
WHERE table_name = UPPER('Geo_Continents')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Geo_Continents'
-- --------------------
SELECT 
...
DECLARE
  ...
  END;
BEGIN
    ExecSql('ALTER TABLE Geo_Regions DROP CONSTRAINT Geo_Regions_fk2');
END;
/
...
-- DROP INDEX  ... Dropped when table is dropped.
	ExecSql('DROP TABLE Geo_Continents');
END;
/
...
CREATE TABLE Geo_Continents (
    ...
    -- Unique Key Columns
    Continent_Name VARCHAR2(30) NOT NULL
)
PCTFREE 10 PCTUSED 40
COMPRESS FOR ALL OPERATIONS
;
...
------------------------------------------------------------------
COMMENT ON TABLE Geo_Continents IS '';

-- Run this after creating the table to generate a list of table column comments:
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Geo_Continents');

COMMENT ON COLUMN Geo_Continents.Code IS 'Pk';
COMMENT ON COLUMN Geo_Continents.Continent_Name IS '';

------------------------------------------------------------------
--PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Geo_Continents  MODIFY (Code DEFAULT 0);
...
------------------------------------------------------------------
-- Primary Key Index
CREATE UNIQUE INDEX Geo_Continents_pk ON Geo_Continents (Code);

------------------------------------------------------------------
--PROMPT '-- (ALTER TABLE) Add Constraints for this table --'
------------------------------------------------------------------
ALTER TABLE Geo_Continents ADD CONSTRAINT Geo_Continents_pk PRIMARY KEY (Code) USING INDEX;
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Geo_Continents, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Geo_Continents, use "ON DELETE SET NULL".
--  If you want Geo_Continents to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
-- ALTER TABLE Geo_Continents ADD CONSTRAINT Geo_Continents_fk1 FOREIGN KEY (ColumnNameFK1) REFERENCES ReferencedTableName1 (ReferencedPK1) [ON DELETE SET NULL];
...
------------------------------------------------------------------
CREATE OR REPLACE TRIGGER Geo_Continents_tr1 BEFORE INSERT OR UPDATE ON Geo_Continents FOR EACH ROW
DECLARE
BEGIN
...
------------------------------------------------------------------
-- GRANT SELECT ON Geo_Continents TO ReadOnlyRole;
-- GRANT SELECT, DELETE, UPDATE, INSERT ON Geo_Continents TO ReadWriteRole;
...
DECLARE
  ...
  END;
BEGIN
    ExecSql('ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_fk2 FOREIGN KEY (Continent_Code) REFERENCES Geo_Continents (Code) ON DELETE SET NULL');
END;
/
