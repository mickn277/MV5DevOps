...
--
-- Purpose:
--  This script creates table Geo_Places, a new table in the database.
--  Foreign Keys on this table Geo_Places reference ReferencedTableName1, ReferencedTableName2 etc.
--  Foreign Keys on ReferencingTableName1, ReferencingTableName2 reference this table Geo_Places.
--  
...
WHERE rdcn.owner = USER 
    AND rdcn.table_name = UPPER('Geo_Places')
...
DECLARE
    v_Exists NUMBER(1) := 0;
BEGIN 
    SELECT DECODE(table_name, null, 0, -1) TABLE_EXISTS INTO V_Exists FROM dual LEFT JOIN user_tables ON (table_name=UPPER('Geo_Places'));
    IF (v_Exists=-1) THEN
...
WHERE table_name = UPPER('Geo_Places')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Geo_Places'
-- --------------------
SELECT 
...
CREATE TABLE Geo_Places (
    ...
    ISO_Numeric3_Code CHAR(3),
    ISO_TopLevel_Domain VARCHAR2(3),
    --
    Name_Alt1 VARCHAR2(100),
    Name_Alt2 VARCHAR2(100),
    Name_Alt3 VARCHAR2(100),
    Latitude NUMBER(12,7),
    Longitude NUMBER(12,7),
    Start_Dt DATE,
    End_Dt DATE,
    Language_First_Code VARCHAR2(6),
    Language_Second_Code VARCHAR2(6),
    Language_Third_Code VARCHAR2(6),
...
--ALTER TABLE Geo_Places ADD (currency_code CHAR(3));
------------------------------------------------------------------
...
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Geo_Places');

COMMENT ON COLUMN Geo_Places.Code IS 'PK';

-- COMMENT ON COLUMN Geo_Places.ColumnNameUK1 IS 'UK 1of2';
-- COMMENT ON COLUMN Geo_Places.ColumnNameUK2 IS 'UK 2of2';
-- COMMENT ON COLUMN Geo_Places.Parent_Code IS 'FK1';
-- COMMENT ON COLUMN Geo_Places.Place_Type IS 'FK2';

COMMENT ON COLUMN Geo_Places.Created_By IS 'Auditing column';
COMMENT ON COLUMN Geo_Places.Created_Dt IS 'Auditing column';
COMMENT ON COLUMN Geo_Places.Changed_By IS 'Auditing column';
COMMENT ON COLUMN Geo_Places.Changed_Dt IS 'Auditing column';

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Geo_Places  MODIFY (Code DEFAULT 0);

-- ALTER TABLE Geo_Places  MODIFY (ColumnNameUK1 DEFAULT 0);
-- ALTER TABLE Geo_Places  MODIFY (ColumnNameUK2 DEFAULT 0);
-- ALTER TABLE Geo_Places  MODIFY (Parent_Code DEFAULT 0);
-- ALTER TABLE Geo_Places  MODIFY (Place_Type DEFAULT 0);

ALTER TABLE Geo_Places  MODIFY (Created_Dt DEFAULT sysdate);
--ALTER TABLE Geo_Places  MODIFY (archive DEFAULT 0);
...
------------------------------------------------------------------
-- Primary Key Index
CREATE UNIQUE INDEX Geo_Places_pk ON Geo_Places (Code);

-- Unique Key Index 1
CREATE UNIQUE INDEX Geo_Places_uk1 ON Geo_Places (Place_Name);

-- Foreign Key Index 1
CREATE INDEX Geo_Places_ix1 ON Geo_Places (Parent_Code);

-- Foreign Key Index 2
CREATE INDEX Geo_Places_ix2 ON Geo_Places (Place_Type);
...
------------------------------------------------------------------
ALTER TABLE Geo_Places ADD CONSTRAINT Geo_Places_pk PRIMARY KEY (Code) USING INDEX;

ALTER TABLE Geo_Places ADD CONSTRAINT Geo_Places_uk1 UNIQUE (Place_Name) USING INDEX;

--ALTER TABLE Geo_Places ADD CONSTRAINT Geo_Places_c1 CHECK (archive IN (0, -1));

--ALTER TABLE Geo_Places ADD CONSTRAINT Geo_Places_c2 CHECK (Place_Type IN ('Value1', 'Value2', 'Value3'));
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Geo_Places, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Geo_Places, use "ON DELETE SET NULL".
--  If you want Geo_Places to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
...
--    v_Geo_Places VARCHAR2(30) := 'Geo_Places';
--BEGIN
--    BEGIN
--        EXECUTE IMMEDIATE 'SELECT max(NVL(id,0))+1 FROM dual LEFT JOIN '||v_Geo_Places||' ON (1=1)' INTO v_START_WITH;
--        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Geo_Places||'_sq START_WITH='||v_START_WITH);
--    EXCEPTION WHEN OTHERS THEN
--        DBMS_OUTPUT.PUT_LINE(SQLERRM);
--    END;
--    BEGIN
--        EXECUTE IMMEDIATE 'DROP SEQUENCE '||v_Geo_Places||'_sq';
--        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Geo_Places||'_sq DROPPED');
--    EXCEPTION WHEN OTHERS THEN
--        NULL;
--    END;
--    EXECUTE IMMEDIATE 'CREATE SEQUENCE '||v_Geo_Places||'_sq MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH '||v_START_WITH||' CACHE 10 NOORDER NOCYCLE';
--    DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Geo_Places||'_sq CREATED');
--END;
--/
...
------------------------------------------------------------------
CREATE OR REPLACE TRIGGER Geo_Places_tr1 BEFORE INSERT OR UPDATE ON Geo_Places FOR EACH ROW
DECLARE
...
------------------------------------------------------------------
--INSERT INTO Geo_Places (Code, ColumnNameUK1, ColumnNameUK2, Parent_Code, Place_Type) VALUES (1, 0, '', 0, '');
--INSERT INTO Geo_Places (Code, ColumnNameUK1, ColumnNameUK2, Parent_Code, Place_Type) VALUES (2, 0, '', 0, '');
--INSERT INTO Geo_Places (Code, ColumnNameUK1, ColumnNameUK2, Parent_Code, Place_Type) VALUES (3, 0, '', 0, '');
--COMMIT;
--
--EXECUTE DBMS_STATS.GATHER_TABLE_STATS(ownname=>USER, tabname=>UPPER('Geo_Places'), cascade=>TRUE, estimate_percent=>DBMS_STATS.AUTO_SAMPLE_SIZE, method_opt=>'for all columns size auto');
...
------------------------------------------------------------------
--GRANT SELECT ON Geo_Places TO ReadOnlyRole;
--GRANT SELECT, DELETE, UPDATE, INSERT ON Geo_Places TO ReadWriteRole;
