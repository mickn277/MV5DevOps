...
--
-- Purpose:
--  This script creates new table Geo_Regions, a new table in the database.
--  Foreign Keys on this new table Geo_Regions reference Geo_Regions, ReferencedTableName2 etc.
--
...
WHERE rdcn.owner = USER 
  AND rdcn.table_name = UPPER('Geo_Regions')
...
WHERE table_name = UPPER('Geo_Regions')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Geo_Regions'
-- --------------------
SELECT 
...
DECLARE
  ...
  END;
BEGIN
    ExecSql('ALTER TABLE Geo_Countries DROP CONSTRAINT Geo_Countries_fk1');
END;
/
...
-- DROP INDEX  ... Dropped when table is dropped.
	--ExecSql('DROP SEQUENCE Geo_Regions_sq');
	ExecSql('DROP TABLE Geo_Regions');
END;
/
...
CREATE TABLE Geo_Regions (
    ...
    Parent_Region_Code CHAR(3),
    -- Foreign Keys
    Continent_Code CHAR(2),
    -- Standard Columns
    Region_Code_Alpha2 CHAR(2),
    Region_Name VARCHAR2(40) NOT NULL,
    Region_Start_Dt DATE,
    Region_End_Dt DATE,
...
------------------------------------------------------------------
COMMENT ON TABLE Geo_Regions IS '';

-- Run this after creating the table to generate a list of table column comments:
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Geo_Regions');

COMMENT ON COLUMN Geo_Regions.Code IS 'PK';

COMMENT ON COLUMN Geo_Regions.Parent_Region_Code IS 'FK1';
COMMENT ON COLUMN Geo_Regions.Region_Name IS '';

COMMENT ON COLUMN Geo_Regions.Created_Dt IS 'Auditing column';
COMMENT ON COLUMN Geo_Regions.Changed_Dt IS 'Auditing column';

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Geo_Regions  MODIFY (Code DEFAULT 0);

-- ALTER TABLE Geo_Regions  MODIFY (ColumnNameUK1 DEFAULT 0);
-- ALTER TABLE Geo_Regions  MODIFY (ColumnNameUK2 DEFAULT 0);
-- ALTER TABLE Geo_Regions  MODIFY (Parent_Region_Code DEFAULT 0);
-- ALTER TABLE Geo_Regions  MODIFY (ColumnNameFK2 DEFAULT 0);

-- ALTER TABLE Geo_Regions  MODIFY (Created_Dt DEFAULT sysdate);
...
------------------------------------------------------------------
-- Primary Key Index
CREATE UNIQUE INDEX Geo_Regions_pk ON Geo_Regions (Code);

-- Foreign Key Index 1
CREATE INDEX Geo_Regions_ix1 ON Geo_Regions (Parent_Region_Code, Code);
CREATE INDEX Geo_Regions_ix2 ON Geo_Regions (Continent_Code);

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add Constraints for this table --'
------------------------------------------------------------------
ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_pk PRIMARY KEY (Code) USING INDEX;

-- ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_uk1 UNIQUE (ColumnNameUK1, ColumnNameUK2) USING INDEX;

-- ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_c1 CHECK (ColumnName IN ('Value1', 'Value2', 'Value3'));

-- ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_c2 CHECK (ColumnName [<|>|=|!=] Value);

--ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_c3 CHECK (Archive IN (-1, 0));
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Geo_Regions, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Geo_Regions, use "ON DELETE SET NULL".
--  If you want Geo_Regions to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_fk1 FOREIGN KEY (Parent_Region_Code) REFERENCES Geo_Regions (Code) ON DELETE SET NULL;
ALTER TABLE Geo_Regions ADD CONSTRAINT Geo_Regions_fk2 FOREIGN KEY (Continent_Code) REFERENCES Geo_Continents (Code) ON DELETE SET NULL;
...
------------------------------------------------------------------
CREATE OR REPLACE TRIGGER Geo_Regions_tr1 BEFORE INSERT OR UPDATE ON Geo_Regions FOR EACH ROW
DECLARE
...
------------------------------------------------------------------
-- INSERT INTO Geo_Regions (Code, ColumnNameUK1, ColumnNameUK2, Parent_Region_Code, ColumnNameFK2) VALUES (1, 0, '', 0, '');
-- INSERT INTO Geo_Regions (Code, ColumnNameUK1, ColumnNameUK2, Parent_Region_Code, ColumnNameFK2) VALUES (2, 0, '', 0, '');
-- INSERT INTO Geo_Regions (Code, ColumnNameUK1, ColumnNameUK2, Parent_Region_Code, ColumnNameFK2) VALUES (3, 0, '', 0, '');
-- COMMIT;
...
------------------------------------------------------------------
-- GRANT SELECT ON Geo_Regions TO ReadOnlyRole;
-- GRANT SELECT, DELETE, UPDATE, INSERT ON Geo_Regions TO ReadWriteRole;
...
DECLARE
  ...
  END;
BEGIN
  ExecSql('ALTER TABLE Geo_Countries ADD CONSTRAINT Geo_Countries_fk1 FOREIGN KEY (Global_Region_Code) REFERENCES Geo_Regions (Code) ON DELETE SET NULL');
END;
/
