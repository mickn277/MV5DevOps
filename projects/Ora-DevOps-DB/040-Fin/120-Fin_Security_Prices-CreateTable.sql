...
--
-- Purpose:
--  This script creates table Fin_Security_Prices, a new table in the database.
--  Foreign Keys on this table Fin_Security_Prices reference ReferencedTableName1, ReferencedTableName2 etc.
--  Foreign Keys on ReferencingTableName1, ReferencingTableName2 reference this table Fin_Security_Prices.
--  
...
WHERE rdcn.owner = USER 
    AND rdcn.table_name = UPPER('Fin_Security_Prices')
...
DECLARE
    v_Exists NUMBER(1) := 0;
BEGIN 
    SELECT DECODE(table_name, null, 0, -1) TABLE_EXISTS INTO V_Exists FROM dual LEFT JOIN user_tables ON (table_name=UPPER('Fin_Security_Prices'));
    IF (v_Exists=-1) THEN
...
WHERE table_name = UPPER('Fin_Security_Prices')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Fin_Security_Prices'
-- --------------------
SELECT 
...
DECLARE
    ...
    END;
BEGIN
	ExecSql('DROP TABLE Fin_Security_Prices');
END;
/
...
PARTITION BY RANGE (Security_Code) 
  (PARTITION P01 VALUES LESS THAN ('0'),
   ...
   PARTITION P04 VALUES LESS THAN ('B'),
   PARTITION P05 VALUES LESS THAN ('F'),
   PARTITION P06 VALUES LESS THAN ('J'),
   PARTITION P07 VALUES LESS THAN ('N'),
   PARTITION P08 VALUES LESS THAN ('R'),
   PARTITION P09 VALUES LESS THAN ('V'),
...
------------------------------------------------------------------
COMMENT ON TABLE Fin_Security_Prices IS '';

-- Run this after creating the table to generate a list of table column comments:
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Fin_Security_Prices');

COMMENT ON COLUMN Fin_Security_Prices.Security_Code IS 'PK 1 of 2';
COMMENT ON COLUMN Fin_Security_Prices.Price_Dt IS 'PK 2 of 2';
COMMENT ON COLUMN Fin_Security_Prices.Close  IS 'Required. Stock charts can work with just closing price. If there is only a closing, median or average price, it goes in close.';

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Fin_Security_Prices  MODIFY (ColumnNamePK DEFAULT 0);
...
------------------------------------------------------------------
ALTER TABLE Fin_Security_Prices ADD CONSTRAINT Fin_Security_Prices_pk PRIMARY KEY (Security_Code, Price_Dt) USING INDEX;

--ALTER TABLE Fin_Security_Prices ADD CONSTRAINT Fin_Security_Prices_c1 CHECK (Year <= 2200);
--ALTER TABLE Fin_Security_Prices ADD CONSTRAINT Fin_Security_Prices_c1 CHECK (Month >= 1 AND Month <= 12);
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Fin_Security_Prices, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Fin_Security_Prices, use "ON DELETE SET NULL".
--  If you want Fin_Security_Prices to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
...
------------------------------------------------------------------
--GRANT SELECT ON Fin_Security_Prices TO ReadOnlyRole;
--GRANT SELECT, DELETE, UPDATE, INSERT ON Fin_Security_Prices TO ReadWriteRole;
...
--    END;
--BEGIN
--    ExecSql('ALTER TABLE ReferencingTableName2 ADD CONSTRAINT ReferencingTableName2_fk11 FOREIGN KEY (ReferencingColumnName2) REFERENCES Fin_Security_Prices (ColumnNamePK) [ON DELETE CASCADE]');
--END;
--/
