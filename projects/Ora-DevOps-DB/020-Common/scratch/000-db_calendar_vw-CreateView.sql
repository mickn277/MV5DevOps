--DROP VIEW db_calendar_vw;
CREATE OR REPLACE VIEW db_calendar_vw AS 
...
FROM ( SELECT TRUNC (ADD_MONTHS (SYSDATE, -60), 'yy') - 1 + LEVEL AS mydate
           FROM dual
         CONNECT BY LEVEL <= (SELECT   TRUNC (ADD_MONTHS (SYSDATE, 60), 'yy')
                                     - TRUNC (ADD_MONTHS (SYSDATE, -24), 'yy')
                                FROM DUAL
                             )
       );