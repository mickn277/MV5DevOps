...
--
-- Purpose:
--  This script creates table Fin_Web_Data_Providers, a new table in the database.
--  Foreign Keys on this table Fin_Web_Data_Providers reference ReferencedTableName1, ReferencedTableName2 etc.
--  Foreign Keys on ReferencingTableName1, ReferencingTableName2 reference this table Fin_Web_Data_Providers.
--  
...
WHERE rdcn.owner = USER 
    AND rdcn.table_name = UPPER('Fin_Web_Data_Providers')
...
DECLARE
    v_Exists NUMBER(1) := 0;
BEGIN 
    SELECT DECODE(table_name, null, 0, -1) TABLE_EXISTS INTO V_Exists FROM dual LEFT JOIN user_tables ON (table_name=UPPER('Fin_Web_Data_Providers'));
    IF (v_Exists=-1) THEN
...
WHERE table_name = UPPER('Fin_Web_Data_Providers')
ORDER BY table_name, column_id;

-- --------------------
PROMPT 'Constraints on Fin_Web_Data_Providers'
-- --------------------
SELECT 
...
--DROP INDEX  ... Dropped when table is dropped.
	ExecSql('DROP SEQUENCE Fin_Web_Data_Providers_sq');
	ExecSql('DROP TABLE Fin_Web_Data_Providers');
END;
/
...
CREATE TABLE Fin_Web_Data_Providers (
    -- Primary Key Column
    id NUMBER (11,0) NOT NULL,
    -- Unique Key Columns
    data_provider_name VARCHAR2(100) NOT NULL,
    page_url VARCHAR2 (255) NOT NULL,
    cost_aud NUMBER(7,2),  -- Max Value 99999.99
    cost_days NUMBER(4),
    cost_updated_dt DATE,
    usefullness_rating NUMBER(1),
    ease_of_use_rating NUMBER(1),
    --primary_country_code CHAR(3),
    process_manual_download VARCHAR2(255),
    process_automated_download VARCHAR2(255),
...
/*
ALTER TABLE Fin_Web_Data_Providers drop (
country_code
);
*/
------------------------------------------------------------------
...
------------------------------------------------------------------
COMMENT ON TABLE Fin_Web_Data_Providers IS '';

-- Run this after creating the table to generate a list of table column comments:
--SELECT '-- COMMENT ON COLUMN '||LOWER(table_name)||'.'||LOWER(column_name)||' IS '''';' "STATEMENTS" FROM user_tab_columns WHERE table_name = UPPER('Fin_Web_Data_Providers');

COMMENT ON COLUMN Fin_Web_Data_Providers.id IS 'PK';

-- COMMENT ON COLUMN Fin_Web_Data_Providers.page_url IS 'UK 1of2';
-- COMMENT ON COLUMN Fin_Web_Data_Providers.data_provider_name IS 'UK 2of2';
--COMMENT ON COLUMN Fin_Web_Data_Providers.primary_country_code IS 'FK1 Country Code of primary country the data provider relates to';
-- COMMENT ON COLUMN Fin_Web_Data_Providers.ColumnNameFK2 IS 'FK2';

COMMENT ON COLUMN Fin_Web_Data_Providers.Created_By IS 'Auditing column';
COMMENT ON COLUMN Fin_Web_Data_Providers.Created_Dt IS 'Auditing column';
COMMENT ON COLUMN Fin_Web_Data_Providers.Changed_By IS 'Auditing column';
COMMENT ON COLUMN Fin_Web_Data_Providers.Changed_Dt IS 'Auditing column';

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add the Column Defaults for this table --'
------------------------------------------------------------------
-- ALTER TABLE Fin_Web_Data_Providers  MODIFY (id DEFAULT 0);

-- ALTER TABLE Fin_Web_Data_Providers  MODIFY (ColumnNameUK1 DEFAULT 0);
-- ALTER TABLE Fin_Web_Data_Providers  MODIFY (ColumnNameUK2 DEFAULT 0);
-- ALTER TABLE Fin_Web_Data_Providers  MODIFY (ColumnNameFK1 DEFAULT 0);
-- ALTER TABLE Fin_Web_Data_Providers  MODIFY (ColumnNameFK2 DEFAULT 0);

ALTER TABLE Fin_Web_Data_Providers  MODIFY (Created_Dt DEFAULT sysdate);
ALTER TABLE Fin_Web_Data_Providers  MODIFY (archive DEFAULT 0);
...
------------------------------------------------------------------
-- Primary Key Index
CREATE UNIQUE INDEX Fin_Web_Data_Providers_pk ON Fin_Web_Data_Providers (id);

-- Unique Key Index 1
-- CREATE UNIQUE INDEX Fin_Web_Data_Providers_uk1 ON Fin_Web_Data_Providers (ColumnNameUK1, ColumnNameUK2);

-- Foreign Key Index 1
--CREATE INDEX Fin_Web_Data_Providers_ix1 ON Fin_Web_Data_Providers (primary_country_code);

-- Foreign Key Index 2
-- CREATE INDEX Fin_Web_Data_Providers_ix2 ON Fin_Web_Data_Providers (ColumnNameFK2);

-- Tuning Index 3
-- CREATE INDEX Fin_Web_Data_Providers_ix3 ON Fin_Web_Data_Providers (ColumnName, ColumnName) [TABLESPACE TablespaceName_index];

------------------------------------------------------------------
PROMPT '-- (ALTER TABLE) Add Constraints for this table --'
------------------------------------------------------------------
ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_pk PRIMARY KEY (id) USING INDEX;

-- ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_uk1 UNIQUE (ColumnNameUK1, ColumnNameUK2) USING INDEX;

-- ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_c1 CHECK (archive IN (0, -1));

-- ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_c2 CHECK (ColumnName IN ('Value1', 'Value2', 'Value3'));

-- ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_c3 CHECK (ColumnName [<|>|=|!=] Value);
...
--  If you want any records in ReferencedTableName(n) to cascade delete records into Fin_Web_Data_Providers, use "ON DELETE CASCADE".
--  If you want any records in ReferencedTableName(n) to delete records without deleting records in Fin_Web_Data_Providers, use "ON DELETE SET NULL".
--  If you want Fin_Web_Data_Providers to lock ReferencedTableName(n) from deleting records, leave blank.
------------------------------------------------------------------
ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_fk1 FOREIGN KEY (primary_country_code) REFERENCES geo_countries (code) ON DELETE SET NULL;

-- ALTER TABLE Fin_Web_Data_Providers ADD CONSTRAINT Fin_Web_Data_Providers_fk2 FOREIGN KEY (ColumnNameFK2) REFERENCES ReferencedTableName2 (ReferencedPK2) [ON DELETE CASCADE];

------------------------------------------------------------------
PROMPT '-- (CREATE SEQUENCE) Create the Sequence for this table --'
------------------------------------------------------------------
-- Simple Create Sequence:
CREATE SEQUENCE Fin_Web_Data_Providers_sq 
...
DECLARE
    ...
    v_Fin_Web_Data_Providers VARCHAR2(30) := 'Fin_Web_Data_Providers';
BEGIN
    BEGIN
        EXECUTE IMMEDIATE 'SELECT max(NVL(id,0))+1 FROM dual LEFT JOIN '||v_Fin_Web_Data_Providers||' ON (1=1)' INTO v_START_WITH;
        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Web_Data_Providers||'_sq START_WITH='||v_START_WITH);
    EXCEPTION WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE(SQLERRM);
    END;
    BEGIN
        EXECUTE IMMEDIATE 'DROP SEQUENCE '||v_Fin_Web_Data_Providers||'_sq';
        DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Web_Data_Providers||'_sq DROPPED');
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
    EXECUTE IMMEDIATE 'CREATE SEQUENCE '||v_Fin_Web_Data_Providers||'_sq MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH '||v_START_WITH||' CACHE 10 NOORDER NOCYCLE';
    DBMS_OUTPUT.PUT_LINE('SEQUENCE '||v_Fin_Web_Data_Providers||'_sq CREATED');
END;
/
...
------------------------------------------------------------------
CREATE OR REPLACE TRIGGER Fin_Web_Data_Providers_tr1 BEFORE INSERT OR UPDATE ON Fin_Web_Data_Providers FOR EACH ROW
DECLARE
...
BEGIN
    ...
    IF INSERTING THEN
        IF (:NEW.id IS NULL) THEN
            SELECT Fin_Web_Data_Providers_sq.NEXTVAL INTO :NEW.id FROM dual; 
        END IF;
    ...
    IF (trg_pkg.comp(:OLD.cost_aud, :NEW.cost_aud)) THEN
        :NEW.cost_updated_dt := sysdate;
    END IF;
END;
/
...
------------------------------------------------------------------
--INSERT INTO Fin_Web_Data_Providers (id, ColumnNameUK1, ColumnNameUK2, ColumnNameFK1, ColumnNameFK2) VALUES (1, 0, '', 0, '');
--INSERT INTO Fin_Web_Data_Providers (id, ColumnNameUK1, ColumnNameUK2, ColumnNameFK1, ColumnNameFK2) VALUES (2, 0, '', 0, '');
--INSERT INTO Fin_Web_Data_Providers (id, ColumnNameUK1, ColumnNameUK2, ColumnNameFK1, ColumnNameFK2) VALUES (3, 0, '', 0, '');
--COMMIT;
--
--EXECUTE DBMS_STATS.GATHER_TABLE_STATS(ownname=>USER, tabname=>UPPER('Fin_Web_Data_Providers'), cascade=>TRUE, estimate_percent=>DBMS_STATS.AUTO_SAMPLE_SIZE, method_opt=>'for all columns size auto');
...
------------------------------------------------------------------
--GRANT SELECT ON Fin_Web_Data_Providers TO ReadOnlyRole;
--GRANT SELECT, DELETE, UPDATE, INSERT ON Fin_Web_Data_Providers TO ReadWriteRole;
...
--    END;
--BEGIN
--    ExecSql('ALTER TABLE ReferencingTableName1 ADD CONSTRAINT ReferencingTableName1_fk10 FOREIGN KEY (ReferencingColumnName1) REFERENCES Fin_Web_Data_Providers (id) [ON DELETE SET NULL]');
--    ExecSql('ALTER TABLE ReferencingTableName2 ADD CONSTRAINT ReferencingTableName2_fk11 FOREIGN KEY (ReferencingColumnName2) REFERENCES Fin_Web_Data_Providers (id) [ON DELETE CASCADE]');
--END;
--/
